...
class Websocket {
     ...
     */
    async sendContext(message, silent = false) {
        if (!this.ws) {
            throw new Error("WebSocket not connected. Call connect() first.");
        }
        const payload = {
     ...
     */
    async unregisterActions(actionNames) {
        if (!this.ws) {
            throw new Error("WebSocket not connected. Call connect() first.");
        }
        const payload = {
     ...
     */
    async forceActions(query, actionNames, state = "", ephemeralContext = false, priority = "low") {
        if (!this.ws) {
            throw new Error("WebSocket not connected. Call connect() first.");
        }
        const payload = {
     ...
     */
    async sendActionResult(actionId, success, message = "") {
        if (!this.ws) {
            throw new Error("WebSocket not connected. Call connect() first.");
        }
        const payload = {
