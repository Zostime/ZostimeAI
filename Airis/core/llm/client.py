...
class LLMClient:
    ...
    def chat(
        ...
        try:
            ...
            response_model = response.model if hasattr(response, 'model') else request_params['model']
            tokens_used = response.usage if hasattr(response, 'usage') else None
            ...
            if hasattr(choice.message, 'tool_calls') and choice.message.tool_calls:
                for tool_call in choice.message.tool_calls:
                    try:
                        ...
                        self.logger.warning(f"JSON 解析失败: {tool_call.function.arguments}")
